...
from ...utils.diffusion_utils import list2batch
...
def pad_freqs(original_tensor, target_len):
    ...
    if seq_len != target_len:
        print('----------', 'target_len,seq_len,pad_size',target_len,seq_len,pad_size,s1,s2)
    pad_size = max(target_len - seq_len,0)
...
class WanModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        if self.enable_teacache:
            print("enable_teacache not in train")
            if cond_flag:
                modulated_inp = e
        ...
        if self.enable_teacache:
            ...
            for index, block in enumerate(self.blocks):
                x = block(x, **kwargs)
                if output_features and index + 1 in selected_layers:
                    # x_unpatchify = self.unpatchify(x, grid_sizes, self.dim//4)
                    # features_list.append(list2batch(x_unpatchify))
                    if get_sequence_parallel_state():
                        x_gathered = all_gather(x, dim=1)
                    else:
                        x_gathered = x
                    features_list.append(x_gathered)
        ...
        # unpatchify
        x = self.unpatchify(x, grid_sizes, self.out_dim)

        return [u.float() for u in x]

    def unpatchify(self, x, grid_sizes, c):
        r"""
